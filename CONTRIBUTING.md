...
- **`<scope>`**: (Optional) A short, descriptive word for what part of the code is affected (e.g., `auth`, `starters`).
...
- **`feat`**: A new feature. Increments the **MINOR** version (e.g., `1.2.0`).
  - Example: `feat(starters): add async pipe handling support`
...
- **`style`**: Changes related to code style (e.g., formatting, missing semicolons). No version increment.
  - Example: `style(starters): reformat code according to ESLint rules`
...
- **`test`**: Changes related to testing (e.g., adding missing tests, refactoring tests). No version increment.
  - Example: `test(starters): add unit tests for error handling`
...
- **`BREAKING CHANGE`**: Use this in the footer if the commit introduces a breaking API change, which increments the **MAJOR** version.
  - Example:
    ```
    feat(starters): change pipe API to accept configuration object

    BREAKING CHANGE: The starters now requires a configuration object instead of individual parameters.
    ```

### Example Commit Messages
1. **New Feature**:
   ```
   feat(starters): add support for multiple output formats
   ```
