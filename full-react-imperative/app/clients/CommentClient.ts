 ...
 * Comment Client Type
*/
export type CommentClient = ReturnType<typeof CommentClient>

/**
 * Comment Client
 */
export const CommentClient = ({ httpClient: client }: CommentClientOptions) => {
  const path = '/comments'
  ...
  return {
    ...
    async list(postId: number, limit: number = 10): Promise<Comment[]> {
      ...
      return await client.get<Comment[]>(`${path}/posts/${postId}?${query}`)
    },
    ...
    async create(postId: number, comment: CommentInput) {
      return client.post(`${path}/posts/${postId}`, { ...comment, id: undefined })
    },
